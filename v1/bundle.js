            ...
            // show / hide
              ...
              // field value
                ...
                // <img src="{ expr }">
                  ...
                  if (value === 0 || value && (typeof value === 'undefined' ? 'undefined' : babelHelpers.typeof(value)) !== T_OBJECT) setAttr(dom, attrName, value);
                }
      });
    }
    /**
...
var riot$1 = riot && (typeof riot === 'undefined' ? 'undefined' : babelHelpers.typeof(riot)) === 'object' && 'default' in riot ? riot['default'] : riot;
...
riot$1.tag2('app', '<ul> <li each="{opts.books}"> <a href="{url}"> <img if="{hasCover}" height="{parent.opts.height}" alt="{title}" riot-src="{image}"> <div if="{!hasCover}" riot-style="height: {parent.opts.height}px; width: {Math.floor(parent.opts.height / 1.3)}px"> <span>{title}</span> </div> </a> </li> </ul>', 'app,[riot-tag="app"] { display: block; font-family: sans-serif; } app ul,[riot-tag="app"] ul { border: 0; padding: 0; margin: 0; } app ul > li,[riot-tag="app"] ul > li { list-style: none; display: inline-block; margin: 5px; box-shadow: 0 0 5px rgba(0, 0, 0, 0.75); vertical-align: bottom; } app ul > li > a,[riot-tag="app"] ul > li > a { display: block; line-height: 0; text-decoration: none; } app div,[riot-tag="app"] div { padding: 10px; background: #fff; color: #666; box-sizing: border-box; overflow: hidden; } app span,[riot-tag="app"] span { display: block; font-size: 10px; font-weight: bold; line-height: 1.3em; }', '', function (opts) {}, '{ }');

var apiServer = 'https://librize.com';
var defaultLimit = 5;
...
route('bookspot/*/latest..', function (bookspot) {
  var q = route.query(),
      ...
      url = apiServer + '/places/' + bookspot + '/place_items.json' + ('?limit=' + limit) + ('&height=' + height);

  fetchJsonp$1(url).then(function (response) {
